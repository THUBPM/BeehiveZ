...
public final class TestArbitrators {
	...
	private static void testMethod(final InterfaceMetricArbitrator arbitrator) {
		...
		System.out.println("Performing Arbitrartion with: "
				+ arbitrator.getShortDescriptionString());
		...
		for (String[] testCase : testCases) {
			final float result = arbitrator.getArbitrationScore(testCase[0],
					testCase[1]);
			System.out.println(df.format(result) + " for \"" + testCase[0]
					+ "\" vs \"" + testCase[1] + "\"");
		}
	}
}
