...
public class MetricHandler {
	...
	public static ArrayList<String> GetMetricsAvailable() {
		...
		try {
			tosubclass = Class
					.forName("uk.ac.shef.wit.simmetrics.similaritymetrics.AbstractStringMetric");
				...
				while (e.hasMoreElements()) {
					...
					if (entryname.startsWith(starts)
						...
						try {
							...
							assert tosubclass != null;
							if (tosubclass.isInstance(o)) {
								outputVect.add(classname.substring(classname
										.lastIndexOf('.') + 1));
							}
						} catch (ClassNotFoundException cnfex) {
							System.err.println(cnfex);
	...
	public static AbstractStringMetric createMetric(String metricName) {
		...
		try {
			Class<?> metricDefinition = Class
					.forName("uk.ac.shef.wit.simmetrics.similaritymetrics."
							+ metricName);
