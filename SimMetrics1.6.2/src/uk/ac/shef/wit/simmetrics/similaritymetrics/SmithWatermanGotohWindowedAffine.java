...
public class SmithWatermanGotohWindowedAffine extends AbstractStringMetric
	...
	public float getSimilarityTimingEstimated(final String string1,
		...
		// increment
		// 0 15.62 62.5 179.5 360 609 891 1297 1640 2125 2688 3297 3984 4766
		// 5485 6437 7313 8312 9391 10500 12704 12938 14359 15704 17266 18844
		// 20360 23547 23845 25688 27656 29532 32048 33891 35844 37938 40251
		// 42610 45001 47407 50142 52266 55314 57970 60782 63814 66470 70376
		// 72767 75861 79283 82564 85814 89408 92658 96283 100080 103283 107518
		// 111033
		final float str1Length = string1.length();
		final float str2Length = string2.length();
		return ((str1Length * str2Length * windowSize) + (str1Length
				* str2Length * windowSize))
				* ESTIMATEDTIMINGCONST;
	}

	/**
	 * gets the similarity of the two strings using Smith-Waterman-Gotoh
	 * distance.
	 * 
	 * @param string1
	 * @param string2
	...
	/**
	 * implements the Smith-Waterman-Gotoh distance function //see
	 * http://www.gen.tcd.ie/molevol/nwswat.html for details.
	 * 
	 * @param s
	 * @param t
	 * 
	...
	public float getUnNormalisedSimilarity(final String s, final String t) {
		...
		for (i = 0; i < n; i++) {
			...
			if (i == 0) {
				...
				for (int k = windowStart; k < i; k++) {
					maxGapCost = Math.max(maxGapCost, d[i - k][0]
							- gGapFunc.getCost(s, i - k, i));
				}
		...
		for (j = 0; j < m; j++) {
			...
			if (j == 0) {
				...
				for (int k = windowStart; k < j; k++) {
					maxGapCost = Math.max(maxGapCost, d[0][j - k]
							- gGapFunc.getCost(t, j - k, j));
				}
		...
		for (i = 1; i < n; i++) {
			for (j = 1; j < m; j++) {
				...
				for (int k = windowStart; k < i; k++) {
					maxGapCost1 = Math.max(maxGapCost1, d[i - k][j]
							- gGapFunc.getCost(s, i - k, i));
				}
				...
				for (int k = windowStart; k < j; k++) {
					maxGapCost2 = Math.max(maxGapCost2, d[i][j - k]
							- gGapFunc.getCost(t, j - k, j));
				}
