...
public final class TagLink extends AbstractStringMetric implements Serializable {
	...
	public TagLink() {
		...
		System.out
				.println("WARNING - this metric is not recomended for fast processing it has been added \n"
						+ " by a third party into the library and from the source is an extremely \n"
						+ " unoptermised process the library author does not recomend its usage \n"
						+ " if you do take the time to perfect this code I will gladly update its\n"
						+ " source - thanks Sam");
	...
	public String getSimilarityExplained(String T, String U) {
		...
		if (T.equals(U)) {
			...
			for (int i = 0; i < tIdfArray.length; i++) {
				buff.append(tTokens[i]).append("\t")
						.append(round(tIdfArray[i])).append("\n");
			}
			buff.append("\nUj\taj(Uj)\n");
			for (int i = 0; i < uIdfArray.length; i++) {
				buff.append(uTokens[i]).append("\t")
						.append(round(uIdfArray[i])).append("\n");
			}
			buff.append("\nScores:\n");
					...
					if (lastTr >= 0 && lastTr < tr) {
						...
						if (innerScore >= 0.0) {
							...
							if (innerScore == 1.0) {
								...
								MR = ((TagLinkToken) characterBasedStringMetric)
										.getMatched();
							}
							MR = MR / minStringSize;
							float IDF = tIdfArray[t] * uIdfArray[u], weight = (IDF + MR) / 2.0f;
							if (innerScore == 1) {
								lastTr = tr;
							}
							buff.append(tTok).append("\t").append(uTok).append(
									"\t").append(round(innerScore))
									.append("\t").append(round(IDF)).append(
											"\t").append(round(MR))
									.append("\t").append(
											round(innerScore * weight)).append(
											"\n");
			...
			for (Object aCandidateList : candidateList) {
				...
				if ((!tMap.containsKey(tPos)) && (!uMap.containsKey(uPos))) {
					...
					tMap.put(tPos, null);
					uMap.put(uPos, null);
					buff.append(tTokens[(tPos)]).append("\t").append(
							uTokens[(uPos)]).append("\t").append(
							round(tokenScore)).append("\n");
				}
			}
	...
	/**
	 * getShortDescriptionString returns the name and parameters of this string
	 * metric
	 * 
	 * @return String
	 */
