 ...
 * test of metrics available.
 * 
 * Date: 24-Mar-2004 Time: 11:09:08
 * 
...
public final class TestMetrics {
	...
	/**
	 * the min time each test is performed for to get an accurate averaged
	...
	private static void Usage() {
		System.out
				.println("Usage: testMethod \"String1 to Test\" \"String2 to test\"");
	...
	private static void testMethod(
			final ArrayList<AbstractStringMetric> metricVector,
			final String[] args) {
		...
		for (int i = 0; i < metricVector.size(); i++) {
			System.out.println("m" + (i + 1) + " "
					+ ((metricVector.get(i))).getShortDescriptionString());
		}
		System.out.println();
		...
		if (!useCmdArgs) {
			...
			for (int i = 0; i < testCases.length; i++) {
				System.out.println("t" + (i + 1) + " \"" + testCases[i][0]
						+ "\" vs \"" + testCases[i][1] + "\"");
			}
			System.out.println();
		} else {
		...
		for (int i = 0; i < metricVector.size(); i++) {
			...
			if (testTimingComplexity) {
				...
				for (int len = 1; len < TESTMAXLENGTHTIMINGTEST; len += TESTMAXLENGTHTIMINGSTEPSIZE) {
					...
					while (timeTaken < TESTTIMINGMILLISECONDSPERTEST) {
						timeTaken += metric.getSimilarityTimingActual(input1,
								input1);
						iterations++;
						metricTests++;
					}
					System.out.print(df.format((float) timeTaken
							/ (float) iterations)
							+ "\t");
				}
			} else if (!useCmdArgs) {
				...
				for (String[] testCase : testCases) {
					...
					System.out.print(df.format(result) + " ("
							+ df.format((float) timeTaken / (float) iterations)
							+ ")\t");
				}
			} else {
				...
				int iterations = 0;
				while (timeTaken < 250) {
					timeTaken += metric.getSimilarityTimingActual(args[0],
							args[1]);
					iterations++;
					metricTests++;
				}
			...
			System.out.print("\t(" + metric.getShortDescriptionString()
					+ ") - testsSoFar = " + metricTests + "\n");
		}
		// output time taken
		totalTime = (System.currentTimeMillis() - totalTime);
		System.out.println("\nTotal Metrics Tests = " + metricTests + " in "
				+ totalTime + "ms\t\t meaning "
				+ df.format((float) metricTests / (float) totalTime)
