...
/**
 * 
 * Description: uk.ac.shef.wit.simmetrics.TestEstimatedTimes tests the estimated
 * times of string metrics. Date: 26-Apr-2004 Time: 16:26:53
 * 
...
public final class TestEstimatedTimes {
	 ...
	 *            arguments vector
	 */
	private static void testMethod(final Vector metricVector,
			final String[] args) {
		...
		for (int i = 0; i < metricVector.size(); i++) {
			System.out.println("m"
					+ (i + 1)
					+ " "
					+ ((AbstractStringMetric) (metricVector.get(i)))
		...
		for (int i = 0; i < metricVector.size(); i++) {
			final AbstractStringMetric metric = (AbstractStringMetric) metricVector
					.get(i);
			...
			for (int len = 1; len < TESTMAXLENGTHTIMINGTEST; len += TESTMAXLENGTHTIMINGSTEPSIZE) {
				...
				System.out.print(df.format((float) timeTaken
						/ (float) iterations)
						+ "("
						+ metric.getSimilarityTimingEstimated(input1, input1)
						+ ")\t");
			}
