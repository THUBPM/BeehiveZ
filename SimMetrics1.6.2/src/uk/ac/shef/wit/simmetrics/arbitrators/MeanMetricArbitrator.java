...
/**
 * Package: uk.ac.shef.wit.simmetrics.arbitrators Description:
 * MeanMetricArbitrator implements a simple arbitrator meaning the scores
 * between metrics given. Date: 29-Apr-2004 Time: 12:27:48
 * 
...
public final class MeanMetricArbitrator implements InterfaceMetricArbitrator,
	 ...
	 * 
	 * @param arbitrationMetrics
	 *            a vector of InterfaceStringMetric's to be used in the
	 *            arbitration
	 * 
	 *            NB this does not perform any error checking so make sure the
	 *            vector contains the correct type.
	 */
	public void setArbitrationMetrics(
			final ArrayList<InterfaceStringMetric> arbitrationMetrics) {
	 ...
	 * 
	 * @param arbitrationMetric
	 *            an InterfaceStringMetric to add to the arbitrated metrics.
	 */
	public void addArbitrationMetric(
			final InterfaceStringMetric arbitrationMetric) {
	 ...
	 * 
	 * @param arbitrationMetrics
	 *            a vectro of InterfaceStringMetric's to added to those used for
	 *            arbitration.
	 * 
	 *            NB this does not perform any error checking so make sure the
	 *            vector contains the correct type.
	 */
	public void addArbitrationMetrics(
			final ArrayList<InterfaceStringMetric> arbitrationMetrics) {
	 ...
	 * <p/>
	 * This call takes as long as the underlying metrics to perform so should
	 * not be performed in normal circumstances.
	 * 
	 * @param string1
	 *            string 1
	 * @param string2
	 *            string 2
	 * 
	 * @return the actual time in milliseconds taken to perform the
	 *         MetricArbitrator
	 */
	public long getArbitrationTimingActual(final String string1,
	...
	/**
	 * gets the estimated time in milliseconds it takes to perform the
	 * arbitration.
	 * 
	 * @param string1
	 *            string 1
	 ...
	 *         similarity measure
	 */
	public float getArbitrationTimingEstimated(final String string1,
			final String string2) {
		float estimatedTime = 0.0f;
		for (Object aMetricsForArbitration : metricsForArbitration) {
			estimatedTime += ((InterfaceStringMetric) aMetricsForArbitration)
					.getSimilarityTimingEstimated(string1, string2);
		}
	...
	public float getArbitrationScore(final String string1, final String string2) {
		...
		if (metricsForArbitration.size() == 0) {
			...
			for (Object aMetricsForArbitration : metricsForArbitration) {
				score += ((InterfaceStringMetric) aMetricsForArbitration)
