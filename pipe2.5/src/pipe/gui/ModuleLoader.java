...
public class ModuleLoader {
	...
	/*
	 * public static Class importModule(File modFile) { Class modClass = null;
	 * 
	 * if (pFileName.endsWith(Pipe.PROPERTY_FILE_EXTENSION) && modFile.exists()
	 * && modFile.isFile() && modFile.canRead()) { try { prop.load(new
	 * FileInputStream(modFile)); } catch (Exception e) {
	 * System.out.println("Error loading " + pFileName); return null; }
	 * 
	 * ExtFileManager.addSearchPath(modFile.getParentFile()); String
	 * moduleClassName = (String)prop.get("module.class");
	 * 
	 * try { modClass = ExtFileManager.loadExtClass(moduleClassName); if
	 * (!isModule(modClass)) { System.out.println(moduleClassName +
	 * " is not a valid module Class"); return null; } } catch (Exception e) { ;
	 * } } return modClass; }
	 */
	/*
	 * public static Class importExternalModule(File modFile) { Class modClass =
	 * null;
	 * 
	 * if (modFile.exists() && modFile.isFile() && modFile.canRead()) {
	 * ExtFileManager.addSearchPath(modFile.getParentFile()); try { modClass =
	 * ExtFileManager.loadExtClass(modFile); if (!isModule(modClass)) {
	 * System.out.println(modFile.getName() + " is not a valid module Class");
	 * return null; } } catch (Exception e) { ; } } return modClass; }
	 */
