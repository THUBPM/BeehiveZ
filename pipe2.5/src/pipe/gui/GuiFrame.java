...
public class GuiFrame extends JFrame implements ActionListener, Observer {
	...
	private TypeAction annotationAction, arcAction, inhibarcAction,
			...
			selectAction, rateAction, markingAction, deleteTokenAction,
			dragAction;
	...
	private void buildMenus() {
		...
		addMenuItem(exportMenu, exportTNAction = new FileAction("Timenet",
				"Export the net to Timenet format", ""));
		...
		/*
		 * // Example files menu try { URL examplesDirURL =
		 ...
		 * "Example nets");
		 * 
		 * Arrays.sort(nets.toArray(), new Comparator(){ public int
		 * compare(Object one, Object two) { return
		 * ((JarEntry)one).getName().compareTo(((JarEntry)two).getName()); } });
		 * 
		 * if (nets.size() > 0) { JMenu exampleMenu=new JMenu("Example nets");
		 * exampleMenu.setIcon( new
		 * ImageIcon(Thread.currentThread().getContextClassLoader().
		 * getResource(CreateGui.imgPath + "Example.png"))); int index = 0; for
		 * (int i = 0; i < nets.size(); i++){ if
		 * (nets.get(i).getName().toLowerCase().endsWith(".xml")){
		 * addMenuItem(exampleMenu, new ExampleFileAction(nets.get(i), (index <
		 * 10) ?("ctrl " + index) :null)); index++; } }
		 * fileMenu.add(exampleMenu); fileMenu.addSeparator(); } } else { File
		 * examplesDir = new File(examplesDirURL.toURI());
		 *//**
		 ...
		 * project name in Eclipse are property converted to '%20' but the blank
		 * in "Example nets" is not. The following code will do nothing on a
		 * Windows machine or if the logic on OSX changess. I also added a stack
		 * trace so if the problem occurs for another environment (perhaps
		 * multiple blanks need to be manually changed) it can be easily fixed.
		 * DP
		 */
		/*
		 * // examplesDir = new File(new URI(examplesDirURL.toString())); String
		 * dirURLString = examplesDirURL.toString(); int index =
		 * dirURLString.indexOf( " " ); if ( index > 0 ) { StringBuffer sb = new
		 * StringBuffer( dirURLString ); sb.replace( index, index + 1, "%20" );
		 ...
		 * 
		 * Arrays.sort(nets,new Comparator(){ public int compare(Object one,
		 * Object two) { return
		 * ((File)one).getName().compareTo(((File)two).getName()); } });
		 * 
		 ...
		 * // is ignored if (nets.length > 0) { JMenu exampleMenu=new
		 * JMenu("Example nets"); exampleMenu.setIcon( new
		 * ImageIcon(Thread.currentThread().getContextClassLoader().
		 * getResource(CreateGui.imgPath + "Example.png"))); int k = 0; for (int
		 * i = 0; i < nets.length; i++){
		 * if(nets[i].getName().toLowerCase().endsWith(".xml")){
		 * addMenuItem(exampleMenu, new ExampleFileAction(nets[i],
		 * (k<10)?"ctrl " + (k++) :null)); } } fileMenu.add(exampleMenu);
		 * fileMenu.addSeparator(); } } } catch (Exception e) {
		...
		addMenuItem(drawMenu, placeAction = new TypeAction("Place", Pipe.PLACE,
				"Add a place", "P", true));
		addMenuItem(drawMenu, transAction = new TypeAction(
				"Immediate transition", Pipe.IMMTRANS,
				"Add an immediate transition", "I", true));
		addMenuItem(drawMenu, timedtransAction = new TypeAction(
				"Timed transition", Pipe.TIMEDTRANS, "Add a timed transition",
				"T", true));
		addMenuItem(drawMenu, arcAction = new TypeAction("Arc", Pipe.ARC,
				"Add an arc", "A", true));
		addMenuItem(drawMenu, inhibarcAction = new TypeAction("Inhibitor Arc",
				Pipe.INHIBARC, "Add an inhibitor arc", "H", true));
		...
		addMenuItem(drawMenu, markingAction = new TypeAction(
				"Marking Parameter", Pipe.MARKING, "Marking Parameter", "M",
				true));
		...
		addMenuItem(experimentMenu,
				loadExperimentAction = new ExperimentAction("Load experiment",
						"Load an experiment file", ""));
		addMenuItem(experimentMenu,
				experimentEditorAction = new ExperimentAction(
						"Experiment Editor", "Start the experiment editor", ""));
	 ...
	 */
	/*
	 * private void loadExperiment(String path){ Experiment exp = new
	 * Experiment(path,appModel); try{ exp.Load(); }
	 * catch(org.xml.sax.SAXParseException spe) { //if the experiment file does
	 * not fit the schema. JOptionPane.showMessageDialog(GuiFrame.this,
	 * "The Experiment file is not valid."+
	 * System.getProperty("line.separator")+ "Line "+spe.getLineNumber()+": "+
	 * spe.getMessage(), "Experiment Input Error", JOptionPane.ERROR_MESSAGE); }
	...
	class ExperimentAction extends GuiAction {
		...
		public void actionPerformed(ActionEvent e) {
			/*
			 * if (this == loadExperimentAction) { File filePath = new
			 * FileBrowser(CreateGui.userPath).openFile(); if ((filePath !=
			 * null) && filePath.exists() && filePath.isFile() &&
			 * filePath.canRead()) { loadExperiment(filePath.getAbsolutePath());
			 * } } if (this == experimentEditorAction) { // ExperimentEditor ee
			 * = new ExperimentEditor(appModel); //ee.start(); }
			 */
		}
