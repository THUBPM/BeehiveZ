 ...
 *         the cfg files and provide path properties. Modules can now be dropped
 *         into the module folder and automatically loaded on all subsequent
 *         executions of pipe. Also refactored to reduce number of methods
 *         loaded with reflection into the Jtree which were subsequently never
 *         used. Now only loading the run method of each of the modules.
 *         (Jan,2007)
...
public class ModuleManager {
	...
	/**
	 * Returns the directory under which the module properties files will be
	 * found. At present this is the bin/cfg directory.
	 * 
	 * Matthew - modified to access module folder directly
	 */
	public File getModuleDir() {
		File modLocation = new File(ExtFileManager
				.getClassRoot(this.getClass()), System
				.getProperty("file.separator")
				+ "pipe" + System.getProperty("file.separator") + "modules");
	 ...
	 *            module directory dynamically loading all compliant pipe module
	 *            class files.
	 */
	// only load attempt to add .class files
	public Vector getModuleClasses(File rootDir) {
		final ExtensionFilter class_files = new ExtensionFilter(
				Pipe.CLASS_FILE_EXTENSION, Pipe.CLASS_FILE_DESC);
	...
	/**
	 * Method creates and returns a Module management tree. This consists of two
	 * nodes, one resposible for listing all the available modules from the
	 * module directory, and another for admin options such as list refreshing.
	 ...
	 * implemented for other options. When the user clicks on a method node the
	 * method is invoked.
	 * 
	 * Matthew - modified to reduce unnecessary reflection, now only loading the
	 * run method of each module class into the tree
	 */
	private void addClassToTree(Class moduleClass) {
		DefaultMutableTreeNode modNode = null;
		if (installedModules.add(moduleClass)) {
			modNode = new DefaultMutableTreeNode(new ModuleClassContainer(
					moduleClass));
	...
	public JTree getModuleTree() {
		...
		if (JarUtilities.isJarFile(modulesDirURL)) {
			try {
				JarFile jarFile = new JarFile(JarUtilities
						.getJarName(modulesDirURL));
				ArrayList<JarEntry> modules = JarUtilities.getJarEntries(
						jarFile, "modules");

				for (int i = 0; i < modules.size(); i++) {
					if (modules.get(i).getName().toLowerCase().endsWith(
							".class")) {
						Class aModuleClass = ModuleLoader.importModule(modules
								.get(i));
						if (aModuleClass != null) {
		...
		DefaultMutableTreeNode add_modules = new DefaultMutableTreeNode(
				loadNodeString);
	...
	class RemoveModuleAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (o instanceof ModuleMethod) {
				...
				installedModules.remove(((ModuleClassContainer) o)
						.returnClass());
			} else {
		...
		private void showPopupMenu(MouseEvent e) {
			...
			if (selPath != null) {
				...
				if ((nodeObj instanceof ModuleClassContainer)
					...
					TreePath removePath = moduleTree.getPathForLocation(e
							.getX(), e.getY());
					JMenuItem menuItem = new JMenuItem(new RemoveModuleAction(
							removePath));
