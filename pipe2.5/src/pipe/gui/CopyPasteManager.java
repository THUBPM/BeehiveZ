 ...
 * @author Pere Bonet
 */
public class CopyPasteManager extends javax.swing.JComponent implements
		pipe.gui.Zoomable, java.awt.event.MouseListener,
	...
	// paste but still hasn't chosen the position where elements will be
	// pasted).
	...
	private void updateBounds() {
		if (pasteInProgress) {
			setBounds(0, 0, CreateGui.getView().getWidth(), CreateGui.getView()
					.getHeight());
		}
	}
	...
	public void setUpPaste(ArrayList<PetriNetObject> toCopy, GuiView _sourceView) {
				...
				if (pnObject instanceof Arc) {
					...
					if (pnObject instanceof NormalArc) {
						if (((NormalArc) pnObject).hasInvisibleInverse()) {
							arcsToPaste.add(((NormalArc) pnObject).getInverse()
									.copy());
						}
					}
				} else {
		...
		if (ptaToPaste.isEmpty() == false) {
			...
			origin.setLocation(Zoomer.getUnzoomedValue(left, zoom), Zoomer
					.getUnzoomedValue(top, zoom));
	...
	private void endPaste(GuiView view) {
		...
		double despX = Grid.getModifiedX(Zoomer.getUnzoomedValue(pasteRectangle
				.getX(), zoom)
				- origin.getX());
		double despY = Grid.getModifiedY(Zoomer.getUnzoomedValue(pasteRectangle
				.getY(), zoom)
				- origin.getY());
		...
		for (int i = 0; i < arcsToPaste.size(); i++) {
			...
			Arc arc = (Arc) (arcsToPaste.get(i)).paste(despX, despY,
					sourceView != view);
			if (arc != null) {
		...
		for (PetriNetObject pno : ptaToPaste) {
			if ((pno instanceof PlaceTransitionObject)) {
				PlaceTransitionObject pt = ((PlaceTransitionObject) pno)
						.getOriginal().getLastCopy();

				Iterator<Arc> pnoConnectedFromIterator = pt
				...
				while (pnoConnectedFromIterator.hasNext()) {
					...
					Iterator<Arc> pnoConnectedToIterator = pt
					...
					while (pnoConnectedToIterator.hasNext()) {
						...
						if (arc1.getSource().equals(arc2.getTarget())
							...
							if (((NormalArc) arc1).isJoined()) {
								((NormalArc) arc1).setInverse((NormalArc) arc2,
										true);

							} else if (((NormalArc) arc2).isJoined()) {
								((NormalArc) arc2).setInverse((NormalArc) arc1,
										true);
		...
		for (PetriNetObject pno : ptaToPaste) {
			if (pno instanceof PlaceTransitionObject) {
				if (((PlaceTransitionObject) pno).getOriginal() != null) {
					// the Place/Transition is a copy of another Object, so we
					// have to
	...
	private void updateSize(Rectangle pasteRectangle, int zoom, int newZoom) {
		...
		pasteRectangle.setSize((int) (realWidth * Zoomer
				.getScaleFactor(newZoom)), (int) (realHeight * Zoomer
				.getScaleFactor(newZoom)));
	}
