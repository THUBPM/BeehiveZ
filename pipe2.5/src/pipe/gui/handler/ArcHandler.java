...
public class ArcHandler extends PetriNetObjectHandler {
	...
	public JPopupMenu getPopup(MouseEvent e) {
		...
		if (myObject instanceof InhibitorArc) {
			...
			if (((NormalArc) myObject).isJoined()) {
				 ...
				 * e.getPoint())); menuItem.setText("Split Arc Segment");
				 */
				popup.insert(menuItem, popupIndex++);

				menuItem = new JMenuItem(new SplitArcsAction(
						(NormalArc) myObject, true));
				 ...
				 * DeleteBothAction((NormalArc)myObject));
				 * menuItem.setText("Delete Both"); popup.insert(menuItem, 8);
				 */
			} else {
	...
	class SplitArcsAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (joined) {
				CreateGui.getView().getUndoManager().addNewEdit(arc.split());
			} else {
				CreateGui.getView().getUndoManager().addNewEdit(arc.join());
			}
		}
