		...
		case Pipe.INHIBARC:
			...
			if (createInhibitorArc != null) {
				if (!currentObject.getClass().equals(
					...
					Iterator arcsFrom = createInhibitorArc.getSource()
		...
		case Pipe.ARC:
			...
			if (createArc != null) {
				if (currentObject != createArc.getSource()) {
					...
					Iterator arcsFrom = createArc.getSource()
							.getConnectFromIterator();
					...
					if (isNewArc == true) {
						...
						Iterator arcsFromTarget = createArc.getTarget()
								.getConnectFromIterator();
							...
							if (anArc.getTarget() == createArc.getSource()) {
								if (anArc instanceof NormalArc) {
									...
									if (inverse.hasInverse()) {
										// if inverse arc has an inverse arc, it
										// means
										// that createArc is equal to inverse's
										// inverse
										// arc so we only have to increment its
										// weight
										isNewArc = false;
										int weightInverse = inverse
												.getInverse().getWeight();
										undoManager.addNewEdit(inverse
												.getInverse().setWeight(
														++weightInverse));
										createArc.delete();
										inverse.getTransition()
					...
					if (isNewArc == true) {
						...
						undoManager.addEdit(new AddPetriNetObjectEdit(
								createArc, view, model));
						if (inverse != null) {
							undoManager.addEdit(inverse.setInverse(
									(NormalArc) createArc, Pipe.JOIN_ARCS));
						}
					}
