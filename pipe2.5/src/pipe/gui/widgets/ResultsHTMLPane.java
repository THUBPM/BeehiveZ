...
/**
 * This class is used by the analysis modules to display the results of their
 * analysis as HTML.
 * 
 * @author unknown
 ...
 *             if the parameter to the constructor is null and a temporary file
 *             cannot be created.
 * 
 *             Changes: 1) Jan 14, 2007 Changed the constructor so that it
 *             checks for a null parameter. It will get a null parameter if you
 *             draw a net rather than opening an XML file containing the net. In
 *             this case, the code now creates a temporary file so it can have a
 *             valid path. The temp file is never opened.
 */
...
public class ResultsHTMLPane extends JPanel implements HyperlinkListener {
	...
	public ResultsHTMLPane(String path) {
		...
		if (path == null) {
			...
			try {
				defaultPath = File.createTempFile("PIPE", ".xml")
						.getParentFile();
			} catch (IOException e) {
		...
		copyAndSaveButtons = new ButtonBar(new String[] { "Copy", "Save" },
	...
	public void setText(String text) {
		results
				.setText("<html><head><style type=\"text/css\">"
	...
	private ActionListener CopyHandler = new ActionListener() {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				...
				System.out
						.println("Error copying to clipboard, seems it's busy?");
			}
		}
	};
	...
	private ActionListener SaveHandler = new ActionListener() {
		...
		public void actionPerformed(ActionEvent arg0) {
			try {
				FileBrowser fileBrowser = new FileBrowser("HTML file", "html",
						defaultPath.getPath());
				...
				String output = "<html><head><style type=\"text/css\">"
						...
						+ "tr.even{background:#a0a0d0}"
						+ "tr.odd{background:#c0c0f0}"
						+ "td.empty{background:#ffffff}" + "</style>"
						+ results.getText();
				writer.write(output);
				writer.close();
			} catch (Exception e) {
