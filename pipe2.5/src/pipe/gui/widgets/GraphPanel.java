...
public class GraphPanel extends JPanel {
	...
	private void drawAxis(Graphics2D g2) {
		Line2D.Double xAxis = new Line2D.Double(getTranslated(xMin, 0),
				getTranslated(xMax, 0));
		Line2D.Double yAxis = new Line2D.Double(getTranslated(0, yMin),
				getTranslated(0, yMax));
		g2.draw(xAxis);
		g2.draw(yAxis);
	...
	private void drawPoints(Graphics2D g2) {
		...
		for (int i = 0; i < xValues.size(); i++) {
			currentPoint.setLocation(getTranslated(((Double) xValues.get(i))
					.doubleValue(), ((Double) yValues.get(i)).doubleValue()));
	...
	private Shape getPointPlot(Point2D.Double point) {
		return new Rectangle2D.Double(point.x - GraphPointSize, point.y
				- GraphPointSize, GraphPointSize * 2, GraphPointSize * 2);
	}
