...
public class MarkingParameter extends Parameter {
	...
	public void delete() {
		...
		if (places.length > 0) {
			...
			for (int i = 0; i < places.length; i++) {
				undoManager
						.addEdit(((Place) places[i]).clearMarkingParameter());
			}
		}
		super.delete();
	}
	...
	public Parameter copy() {
		return new MarkingParameter(name, value, (int) this.getX(), (int) this
				.getY());
	}

	public Parameter paste(double x, double y, boolean fromAnotherView) {
		return new MarkingParameter(name, value, (int) this.getX()
				+ Grid.getModifiedX(x), (int) this.getY()
				+ Grid.getModifiedY(y));
	}
