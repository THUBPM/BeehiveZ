...
public class RateParameter extends Parameter {
	...
	public Parameter copy() {
		return new RateParameter(name, value, (int) this.getX(), (int) this
				.getY());
	}

	public Parameter paste(double x, double y, boolean fromAnotherView) {
		return new RateParameter(name, value, (int) this.getX()
	...
	public void delete() {
		...
		if (transitions.length > 0) {
			...
			for (int i = 0; i < transitions.length; i++) {
				undoManager.addEdit(((Transition) transitions[i])
						.clearRateParameter());
			}
		}
		super.delete();
