...
public abstract class Note extends PetriNetObject implements Translatable {
	...
	public void updateBounds() {
		...
		bounds
				.setBounds(
						(int) Zoomer.getZoomedValue(originalX, zoom) - 20,
						(int) Zoomer.getZoomedValue(originalY, zoom) - 20,
						(int) ((rectWidth + Pipe.RESERVED_BORDER + Pipe.ANNOTATION_SIZE_OFFSET) * Zoomer
								.getScaleFactor(zoom)) + 20,
						(int) ((rectHeight + Pipe.RESERVED_BORDER + +Pipe.ANNOTATION_SIZE_OFFSET) * Zoomer
								.getScaleFactor(zoom)) + 20);
		setBounds(bounds);
	}
