 ...
 *         the steady state of a general stochastic petri net. The only public
 *         method is solve() which calculates the steady state.
 */
public class SteadyStateSolver {
	...
	public static double[] solve(File rgfile) throws ImmediateAbortException {
		...
		/*
		 * // debug System.out.println("gaussSiedel(Qprime): "); for (int i = 0;
		 * i < Qprime.length; i++) { System.out.println(Qprime[i] + " "); }
		 */
	...
	/**
	 * createSparseMatrix() Read's the state to state transitions from a
	 * reachability graph file and the associated rates and creates the matrix
	 * Q. However, as it is a sparse matrix (lots of zeroes) it creates it in a
	 * more efficient form than an n by n array. Instead it returns an array of
	 * LinkedLists. Each ROW of the array represents a COLUMN of the matrix.
	 * Each object in the linked list represents an element of the matrix as a
	 * record <row, rate>. This way, only non-zero elements are stored. It is
	 * stored column wise as later calculations use the transpose of this
	 * matrix.
	 * 
	 * @param rgfile
	 *            The reachability graph file
	 * @return The sparse matrix Q
	 * @throws ImmediateAbortException
	 */
	private static LinkedList[] createSparseMatrix(File rgfile)
			throws ImmediateAbortException {
		...
		try {
			...
			// element tells you which row it's in. We just need to get the
			// column
			// right.
			...
			for (int record = 0; record < numtransitions; record++) {
				...
				if (current.getFromState() != current.getToState()) {
					...
					Qprime[column]
							.add(new MatrixElement(row, current.getRate()));
					rowsum[row] += current.getRate();
				}
				System.out.print(f
						.format(((double) record / numtransitions) * 100)
						+ "% complete.  \r");
			}
			...
			for (int column = 0; column < numcolumns; column++) {
				Qprime[column].add(new MatrixElement(column,
						0.0 - rowsum[column]));
			}
		...
		/*
		 * //debug for(int i=0; i < numcolumns; i++){ System.out.println("Col: "
		 * + i + " " + Qprime[i]); }
		 */
	...
	/**
	 * gaussSiedel() Solves matrix equations of the form Ax = b for x using the
	 * Gauss-Siedel iterative method. In this case, A is a sparse matrix, b=0
	 * and the sum of all xi = 1
	 * 
	 * @param A
	 *            a sparse matrix
	 * @return
	...
	private static double[] gaussSiedel(LinkedList[] A) {
		...
		while (!plausible) {
			...
			// the steady state values are plausible by checking none of the
			// values
			// are < 0
...
/**
 * @author Nadeem This class is used to create a sparse matrix. It represents
 *         each element in the matrix as a pair of values <row, rate>.
 */
class MatrixElement {
