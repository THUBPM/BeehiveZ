...
public class myTree {
	...
	public myTree(DataLayer data, int[] treeRoot, File reachabilityGraph)
		...
		// between them. They are later combined into one file by the
		// createCGFile
		// method
	...
	public void createCoverabilityGraph(RandomAccessFile outputFile,
		...
		boolean[] enabledTransitions = dataLayer
				.getTransitionEnabledStatusArray(root.markup);
		...
		while (!unprocessednodes.isEmpty()) {
			...
			enabledTransitions = dataLayer
					.getTransitionEnabledStatusArray(currentNode.markup);
			// For each transition
			for (int i = 0; i < enabledTransitions.length; i++) {
				if (enabledTransitions[i] == true) {
					...
					// currentNode.print("\n currentNode.markup is :",
					// currentNode.markup);//debug
					...
					// Create a new node using the new markup vector and attach
					// it to
					// the current node as a child.
					currentNode.children[i] = new myNode(newMarkup,
							currentNode, this, currentNode.depth + 1);

					/*
					 * Now need to (a) check if any omegas (represented by -1)
					 * need to be inserted in the markup vector of the new node,
					 * and (b) if the resulting markup vector has appeared
					 * anywhere else in the tree. We must do (a) before (b) as
					 * we need to know if the new node contains any omegas to be
					 * able to compare it with existing nodes.
					 */
					allOmegas = currentNode.children[i].InsertOmegas();

					// print("\n New Markup (after omegas) is :",
					// newMarkup);//debug
					// System.out.println("afegit fill a la posicio " + i +
					// "; profunditat" + (currentNode.depth +1));

					repeatedNode = (this.root)
							.FindMarkup(currentNode.children[i]);
					...
					if ((!repeatedNode) /* && (!allOmegas) */) {
						try {
							writeNode(currentNode.children[i].id,
									currentNode.children[i].markup, esoFile,
									true);
							this.states++;
						} catch (IOException e) {
							System.err
						...
						if (currentNode.children[i].previousInstance != null) {
							writeEdge(
									currentNode.id,
									currentNode.children[i].previousInstance.id,
									0, i, currentNode.markup, outputFile);
						} else {
							writeEdge(currentNode.id,
									currentNode.children[i].id, 0, i,
		...
		while (!unprocessednodes.isEmpty()) {
			...
			for (int i = 0; i < enabledTransitions.length; i++) {
				if (enabledTransitions[i] == true) {
					...
					try {
						writeNode(currentNode.children[i].id,
								currentNode.children[i].markup, esoFile, true);
						this.states++;
					} catch (IOException e) {
						System.err
								.println("IO problem while writing explored states to file.");
					...
					// if (currentNode.children[i].previousInstance != null) {
					// writeEdge(currentNode.id,
					// currentNode.children[i].previousInstance.id, 0, i,
					// currentNode.markup, outputFile);
					// } else {
					writeEdge(currentNode.id, currentNode.children[i].id, 0, i,
							currentNode.markup, outputFile);
					// }
					// } else {
					// emmagatzemar nova transicio
	...
	/**
	 * writeEdge() Records all the arcs in the reachability graph from state
	 * 'from'.
	 * 
	 * @param from
	 ...
	 */
	private void writeEdge(int from, int to, double rate, int transitionNo,
			int[] markup, RandomAccessFile dataFile)
			throws ImmediateAbortException {
		TransitionRecord newTransition = new TransitionRecord(from, to, rate,
				transitionNo);
		// System.out.println("writeEdge: from S" + from + " to S" + to +
		// "; t =" + transitionNo); //debug
		try {
	...
	private void writeNode(int stateId, int[] marking, RandomAccessFile opfile,
			boolean vanishingStates) throws IOException {
