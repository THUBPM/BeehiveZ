...
public class StateSpaceGenerator {
	...
	public static void generate(DataLayer pnmlData, File reachGraph)
		...
		currentState = new Marking(currentMarking, numStates, isTangible(
		...
		while (!statesQueue.isEmpty()) {
			...
			while (!tansuccessor.isEmpty()) {
				...
				if (!explored(sprime, exploredStates)) {
					currentState = new Marking(sprime, numStates, isTangible(
							pnmlData, sprime));
					numStates++;
					statesQueue.enqueue(currentState);
					addExplored(currentState, exploredStates, esoFile, true);
			...
			if (numTransitions > 400) {
				throw new OutOfMemoryError(
						"The net generates in excess of 20000 states");
			}
		}
		try {
			outputFile.close();
		...
		if (isTangible(pnmlData, currentMarking)) {
			...
			while ((!vanishingStates.isEmpty()) && (attempts != MAX_TRIES)) {
				...
				boolean[] enabledTransitions = pnmlData
						.getTransitionEnabledStatusArray(v.getState());
		...
		while (!tangibleStates.isEmpty()) {
			if (progress == UPDATEAFTER) {
				...
				System.out.print(numtangiblestates
						+ " tangible states generated and "
						+ numtransitionsfired + " transitions fired.\r");
			} else {
				progress++;
			}
	...
	private static boolean isTangible(DataLayer pnmlData, State marking) {
		...
		boolean[] enabledTransitions = pnmlData
				.getTransitionEnabledStatusArray(marking.getState());
	...
	private static int fire(DataLayer pnmlData, State vs, Stack succ) {
		...
		boolean[] enabledTransitions = pnmlData
				.getTransitionEnabledStatusArray(vs.getState());
	...
	private static double rate(DataLayer pnmlData, State s, State sprime) {
		...
		boolean[] marking1EnabledTransitions = pnmlData
				.getTransitionEnabledStatusArray(marking1);
	 ...
	 * @return double - the probability
	 */
	private static double prob(DataLayer pnmlData, State s, State sprime,
			int firedTransition) {
