...
/**
 * @author Matthew Cook after James Bloom/Clare Clark Class used in state space
 *         and GSPN modules for generation of trees and arrays of possible state
 *         spaces
 */
public class myNode {
	...
	/**
	 * Function: void myNode(int[] markingArray, myNode parentNode, myTree
	 * atree) The Node constructor called by a node object
	 */
	...
	/**
	 * Function: int[] fire(int trans) Produces a new markup vector to simulate
	 * the firing of a transition. Destroys the number of tokens shown in CMinus
	 * for a given place and transition, and creates the number of tokens shown
	 * in CPlus. TransIndex refers to the actual transition number ie starting
	 * at 1.
	 */
	...
	/**
	 * Function: void RecursiveExpansion() Undertakes a recursive expansion of
	 * the tree Called on root node from within the tree constructor.
	 */
	public void RecursiveExpansion() throws TreeTooBigException {
		...
		boolean[] enabledTransitions = tree.dataLayer
				.getTransitionEnabledStatusArray(markup);
		...
		for (int i = 0; i < enabledTransitions.length; i++) {
			if (enabledTransitions[i] == true) {
				...
				// Check for safeness. If any of places have > 1 token set
				// variable.
				...
				/*
				 * Now need to (a) check if any omegas (represented by -1) need
				 * to be inserted in the markup vector of the new node, and (b)
				 * if the resulting markup vector has appeared anywhere else in
				 * the tree. We must do (a) before (b) as we need to know if the
				 * new node contains any omegas to be able to compare it with
				 * existing nodes.
				 */
				allOmegas = children[i].InsertOmegas();

				// check if the resulting markup vector has occured anywhere
				// else in the tree
		...
		if (!aTransitionIsEnabled) {
			...
			if (!tree.noEnabledTransitions
					|| tree.pathToDeadlock.length < depth - 1) {
				RecordDeadlockPath();
				tree.noEnabledTransitions = true;
			} else {
				System.err
						.println("Deadlocked node found, but path is not shorter"
								+ " than current path.");
			}
		} else {
	...
	/**
	 * Function: void RecordDeadlockPath() If there is a deadlock, calculates
	 * the path
	 */
	...
	/**
	 * Function: void InsertOmegas() Checks if any omegas need to be inserted in
	 * the places of a given node. Omegas (shown by -1 here) represent unbounded
	 * places and are therefore important when testing whether a petri net is
	 * bounded. This function checks each of the ancestors of a given node.
		...
		while (ancestorNode != tree.root && !insertedOmega) {
			...
			loop: for (int i = 0; i < tree.placeCount; i++) {
				if (markup[i] != -1) {
					...
					// If M(p) for new node strictly greater than M(p) for
					// ancestor
			...
			if (allElementsGreaterOrEqual == true) {
				...
				for (int p = 0; p < tree.placeCount; p++) {
					...
					if (!inhibition) {
						if (markup[p] != -1 && elementIsStrictlyGreater[p]
								&& tree.capacity[p] == 0) {
	...
	/**
	 * Function: boolean FindMarkup(int[] mark) Checks if the markup has occured
	 * previously in the tree. This means previously during the creation of the
	 * tree, and not just on the current branch as a direct ancestor of the
	 * node. Function updates arrays of end nodes stored in the tree class for
	 * the liveness tests. Explore the tree by starting with the root node and
	...
	/**
	 * Function: boolean MarkupCompare(int[] check) Takes two integer arrays
	 * (Markups) and compares the values. Returns a boolean showing the results
	 * of the comparison.
	 */
