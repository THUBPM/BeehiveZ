...
public class ArcPath implements Shape, Cloneable {
	...
	public void createPath() {
		...
		if (getEndIndex() < 2) {
			midPoint.x = (float) ((((ArcPathPoint) pathPoints.get(0))
					.getPoint().x + ((ArcPathPoint) pathPoints.get(1))
					.getPoint().x) * 0.5);
			midPoint.y = (float) ((((ArcPathPoint) pathPoints.get(0))
					.getPoint().y + ((ArcPathPoint) pathPoints.get(1))
					.getPoint().y) * 0.5);
		} else {
			double acc = 0;
			double percent = 0;
		...
		for (int c = 1; c <= endIndex;) {
			...
			if (myCurrentPoint.getPointType() == true) {
				...
				for (k1 = 0; k1 <= (curveEndIndex - curveStartIndex); k1++) {
					x[k1] = (int) ((ArcPathPoint) pathPoints
							.get(curveStartIndex + k1)).getPoint().x;
					y[k1] = (int) ((ArcPathPoint) pathPoints
							.get(curveStartIndex + k1)).getPoint().y;
				}
				x[k1] = x[k1 - 1];
				y[k1] = y[k1 - 1];
				...
				for (int k2 = 1; k2 <= lengthOfCurve; k2++) {
					myCurrentPoint = (ArcPathPoint) pathPoints.get(k2
							+ curveStartIndex);
		...
		for (int c = 1; c <= getEndIndex(); c++) {
			...
			if (myCurrentPoint.getPointType() == false) {
				...
				if (c > 1 && myPreviousPoint.getPointType() == false) {
					myPreviousButOnePoint = (ArcPathPoint) pathPoints
							.get(c - 2);
	...
	private void setEndControlPoints() {
		...
		if (!(getEndIndex() > 0)) {
			...
			myPoint.setControl2(getControlPoint(myPoint.getPoint(), myPoint
				...
				&& ((ArcPathPoint) pathPoints.get(getEndIndex()))
						.getPointType() == true) {
			ArcPathPoint myPoint = (ArcPathPoint) pathPoints.get(getEndIndex());
			ArcPathPoint myLastPoint = (ArcPathPoint) pathPoints
					.get(getEndIndex() - 1);
			float distance = (float) getMod(myPoint.getPoint(), myLastPoint
					.getPoint())
					/ Pipe.ARC_CONTROL_POINT_CONSTANT;
			...
			myPoint.setControl1(getControlPoint(((ArcPathPoint) pathPoints
					.get(0)).getPoint(), myPoint.getControl2(),
					((ArcPathPoint) pathPoints.get(0)).getPoint(), myPoint
							.getControl2()));
		}
	}
		...
		if (getEndIndex() > 0) {
			if (getArc().getTarget() instanceof Transition) {
				return ((ArcPathPoint) pathPoints.get(getEndIndex()))
						.getAngle(((ArcPathPoint) (pathPoints
	...
	public double getStartAngle() {
		if (getEndIndex() > 0) {
			return ((ArcPathPoint) pathPoints.get(0))
					.getAngle(((ArcPathPoint) (pathPoints.get(1)))
							.getControl2());
		}
		return 0;
	}
	...
	public void addPointsToGui(GuiView editWindow) {
		...
		((ArcPathPoint) pathPoints.get(pathPoints.size() - 1))
				.setDraggable(false);
		...
		for (int i = 0; i < pathPoints.size(); i++) {
			...
			if (editWindow.getIndexOf(pathPoint) < 0) {
				...
				if (pathPoint.getMouseWheelListeners().length == 0) {
					pathPoint
							.addMouseWheelListener((MouseWheelListener) pointHandler);
				}
	...
	public void delete() { // Michael: Tells the arc points to remove themselves
		while (!pathPoints.isEmpty()) {
			((ArcPathPoint) pathPoints.get(0)).kill(); // force delete of ALL
			// points
		}
	}
	...
	public String[][] getArcPathDetails() {
		...
		for (int c = 0; c < length; c++) {
			...
			details[c][2] = String.valueOf(((ArcPathPoint) pathPoints.get(c))
					.getPointType());
		}
		return details;
	}
