...
public class PNMatrix {
	...
	public boolean checkCase11() {
		boolean satisfies11 = false; // true means there is an empty set pPlus
		// or pMinus
	...
	public int[] colsToUpdate() {
		...
		for (int i = 0; i < m; i++) {
			...
			if (countpPlus == 1) {
				...
				// if pMinus has cardinality ==1 return all the elements in
				// pPlus reduced by 1 each
				return pPlus;
			}
		}
		return js;
	...
	public PNMatrix eliminateCol(int toDelete) {
		...
		int[] cols = new int[n - 1]; // array of cols which will not be
		// eliminated
		int count = 0;
		...
		// System.out.print("Eliminating column " + toDelete +
		// " from matrix below... keeping columns ");
		// printArray(cols);
		// print(2, 0);
	...
	public int findNonMinimal() {
		...
		PNMatrix X = new PNMatrix(m, 1); // column one, represents first col of
		// comparison
		PNMatrix Y = new PNMatrix(m, 1); // col two, represents rest columns of
		// comparison
	...
	public void linearlyCombine(int k, int[] alpha, int[] j, int[] beta) {
		...
		for (int i = 0; i < n; i++) {
			if (j[i] != 0) {
				// chk = jC[i];
				// System.out.print("\nchk = " + chk + "\n");
				for (int w = 0; w < m; w++) { // for all the elements in a
					// column
