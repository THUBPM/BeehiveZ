...
public class DirectedAcyclicGraphTest extends TestCase {
	...
	private void setUpWithSeed(int vertices, int edges, long seed) {
		randomGraphGenerator = new RepeatableRandomGraphGenerator<Long, DefaultEdge>(
				vertices, edges, seed);
		sourceGraph = new SimpleDirectedGraph<Long, DefaultEdge>(
				DefaultEdge.class);
		randomGraphGenerator.generateGraph(sourceGraph,
				new LongVertexFactory(), null);
	}
	...
	public void testCycleDetectionInRandomGraphBuild() {
		for (int i = 0; i < 50; i++) { // test with 50 random graph
			...
			DirectedAcyclicGraph<Long, DefaultEdge> dag = new DirectedAcyclicGraph<Long, DefaultEdge>(
					DefaultEdge.class);
			SimpleDirectedGraph<Long, DefaultEdge> compareGraph = new SimpleDirectedGraph<Long, DefaultEdge>(
					DefaultEdge.class);
			...
			for (DefaultEdge edge : sourceGraph.edgeSet()) {
				...
				DefaultEdge compareEdge = compareGraph.addEdge(edgeSource,
						edgeTarget);
				CycleDetector<Long, DefaultEdge> cycleDetector = new CycleDetector<Long, DefaultEdge>(
						compareGraph);
			...
			for (Long sourceVertex : compareGraph.vertexSet()) {
				for (DefaultEdge outgoingEdge : compareGraph
						.outgoingEdgesOf(sourceVertex)) {
					Long targetVertex = compareGraph
	...
	public void testTopoIterationOrderLinearGraph() {
		DirectedAcyclicGraph<Long, DefaultEdge> dag = new DirectedAcyclicGraph<Long, DefaultEdge>(
				DefaultEdge.class);
		LinearGraphGenerator<Long, DefaultEdge> graphGen = new LinearGraphGenerator<Long, DefaultEdge>(
				100);
		graphGen.generateGraph(dag, new LongVertexFactory(), null);

		Iterator<Long> internalTopoIter = dag.iterator();
		TopologicalOrderIterator<Long, DefaultEdge> comparTopoIter = new TopologicalOrderIterator<Long, DefaultEdge>(
				dag);
	...
	public void testTopoIterationOrderComplexGraph() {
		for (int seed = 0; seed < 20; seed++) {
			DirectedAcyclicGraph<Long, DefaultEdge> dag = new DirectedAcyclicGraph<Long, DefaultEdge>(
					DefaultEdge.class);
			RepeatableRandomGraphGenerator<Long, DefaultEdge> graphGen = new RepeatableRandomGraphGenerator<Long, DefaultEdge>(
					100, 500, seed);
			graphGen.generateGraph(dag, new LongVertexFactory(), null);

			ConnectivityInspector<Long, DefaultEdge> connectivityInspector = new ConnectivityInspector<Long, DefaultEdge>(
					dag);
	...
	public void _testPerformanceVersusStaticChecking() {
		...
		for (int numVertices = 1024; numVertices <= maxVertices; numVertices *= 2) {
			for (int connectednessFactor = 1; (connectednessFactor <= maxConnectednessFactor)
				...
				for (int seed = 0; seed < trialsPerConfiguration; seed++) { // test
																			// with
																			// random
																			// graph
																			// configurations
					setUpWithSeed(numVertices, numVertices
							* connectednessFactor, seed);
					...
					for (DefaultEdge edge : sourceGraph.edgeSet()) {
						...
						DefaultEdge compareEdge = compareGraph.addEdge(
				...
				System.out.println("vertices = " + numVertices
						+ "  connectednessFactor = " + connectednessFactor
						+ "  trialsPerConfiguration = "
						+ trialsPerConfiguration);
				System.out.println("total static DAG time   =  "
						+ staticDagTime + " ns");
				System.out.println("total dynamic DAG time  =  "
						+ dynamicDagTime + " ns");
				System.out.println();
			}
		}
		...
		for (int numVertices = 64; numVertices <= maxVertices; numVertices *= 2) {
			for (int connectednessFactor = 1; (connectednessFactor <= maxConnectednessFactor)
				...
				for (int seed = 0; seed < trialsPerConfiguration; seed++) { // test
					...
					DirectedAcyclicGraph<Long, DefaultEdge> arrayDag = new DirectedAcyclicGraph<Long, DefaultEdge>(
							DefaultEdge.class,
							new DirectedAcyclicGraph.VisitedArrayImpl(), null);
					DirectedAcyclicGraph<Long, DefaultEdge> arrayListDag = new DirectedAcyclicGraph<Long, DefaultEdge>(
							DefaultEdge.class,
							new DirectedAcyclicGraph.VisitedArrayListImpl(),
							null);
					DirectedAcyclicGraph<Long, DefaultEdge> hashSetDag = new DirectedAcyclicGraph<Long, DefaultEdge>(
							DefaultEdge.class,
							new DirectedAcyclicGraph.VisitedHashSetImpl(), null);
					DirectedAcyclicGraph<Long, DefaultEdge> bitSetDag = new DirectedAcyclicGraph<Long, DefaultEdge>(
							DefaultEdge.class,
							new DirectedAcyclicGraph.VisitedBitSetImpl(), null);
				...
				System.out.println("vertices = " + numVertices
						...
						+ "  trialsPerConfiguration = "
						+ trialsPerConfiguration);
				System.out.println("total array time       =  " + arrayDagTime
						+ " ns");
				System.out.println("total ArrayList time   =  "
						+ arrayListDagTime + " ns");
				System.out.println("total HashSet time     =  "
						+ hashSetDagTime + " ns");
				System.out.println("total BitSet time     =  " + bitSetDagTime
						+ " ns");
				System.out.println();
			}
		}
	...
	private static class LongVertexFactory implements VertexFactory<Long> {
	...
	private static class RepeatableRandomGraphGenerator<V, E> extends
		...
		public void generateGraph(Graph<V, E> graph,
			...
			for (int i = 0; i < numOfEdges; i++) {
				...
				try {
					graph.addEdge(vertices.get(fromVertexId), vertices
							.get(toVertexId));
