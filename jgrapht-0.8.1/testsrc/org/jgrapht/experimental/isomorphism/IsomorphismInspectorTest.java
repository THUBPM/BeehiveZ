...
public class IsomorphismInspectorTest extends TestCase {
	 ...
	 * edgeChecker = null
	 */
	private void assertIsomorphic(Graph<Integer, DefaultEdge>[] graphs,
	...
	private void assertIsomorphic(Graph<Integer, DefaultEdge>[] graphs,
		...
		GraphIsomorphismInspector iso = AdaptiveIsomorphismInspectorFactory
		...
		while (iso.hasNext()) {
			...
			if (false) {
				if (counter == 0) {
					System.out
							.println("Graphs are isomorphic. Iterating over all options:");
				}
	...
	@SuppressWarnings("unchecked")
	private void checkRelation(Graph<Integer, DefaultEdge>[] graphs,
			EquivalenceComparator vertexChecker,
			EquivalenceComparator edgeChecker) {
		Graph<Integer, DefaultEdge> g1 = graphs[0];
		Graph<Integer, DefaultEdge> g2 = graphs[1];
		...
		if (iso.hasNext()) {
			...
			for (Iterator<Integer> iter = vertexSet.iterator(); iter.hasNext();) {
			...
			for (Iterator<DefaultEdge> iter = edgeSet.iterator(); iter
					.hasNext();) {
	...
	public void testWheelGraphAddRemoveParts() {
		...
		Graph<Integer, DefaultEdge> g1 = new DefaultDirectedGraph<Integer, DefaultEdge>(
				DefaultEdge.class);
		Graph<Integer, DefaultEdge> g2 = new DefaultDirectedGraph<Integer, DefaultEdge>(
				DefaultEdge.class);
		WheelGraphGenerator<Integer, DefaultEdge> gen1 = new WheelGraphGenerator<Integer, DefaultEdge>(
		...
		gen1.generateGraph(g2, new IntegerVertexFactory(FIRST_INTEGER_FOR_G2),
				null);
		assertIsomorphic(new Graph[] { g1, g2 }, true);

		// In a wheel , the last vertex is the wheel center!
		g1.removeVertex(new Integer(NUM_OF_VERTEXES_IN_WHEEL)); // delete one
																// vertex from
																// g1
		assertIsomorphic(new Graph[] { g1, g2 }, false);
	...
	public void testLinear4vertexIsomorphicGraph() {
		...
		gen2.generateGraph(g2, new IntegerVertexFactory(5), null); // start
																	// vertex
																	// from
																	// number 6
		assertIsomorphic(new Graph[] { g1, g2 }, true);

		checkRelation(new Graph[] { g1, g2 }, null, null);
	}
	...
	public void testLinear4vertexNonIsomorphicCauseOfVertexEqGroup() {
		LinearGraphGenerator<Integer, DefaultEdge> gen4 = new LinearGraphGenerator<Integer, DefaultEdge>(
				4);
		...
		Graph<Integer, DefaultEdge> g2 = new DefaultDirectedGraph<Integer, DefaultEdge>(
				DefaultEdge.class);
		gen4.generateGraph(g2, new IntegerVertexFactory(1), null); // start
																	// vertex
																	// from
																	// number 2

		Graph<Integer, DefaultEdge> g3 = new DefaultDirectedGraph<Integer, DefaultEdge>(
				DefaultEdge.class);
		gen4.generateGraph(g3, new IntegerVertexFactory(2), null); // start
																	// vertex
																	// from
																	// number 3
		...
		assertIsomorphic(new Graph[] { g1, g2 }, true);
		assertIsomorphic(new Graph[] { g2, g3 }, true);
		assertIsomorphic(new Graph[] { g1, g3 }, true);

		// create a functor according to odd even
		EquivalenceComparator vertexEqChecker = new OddEvenGroupComparator();
		assertIsomorphic(new Graph[] { g1, g2 }, false, vertexEqChecker, null);
		assertIsomorphic(new Graph[] { g2, g3 }, false, vertexEqChecker, null);
		assertIsomorphic(new Graph[] { g1, g3 }, true, vertexEqChecker, null);
	}
	...
	public void testEdgeComparator() {
		...
		Character[] charArray = new Character[] { new Character('A'),
				new Character('B'), new Character('C'), new Character('D') };
		int[][] weigthsArray = new int[][] { { 12, 10, 5 }, { 10, 18, 3 },
				{ 11, 10, 5 } };
		...
		for (int i = 0; i < graphsArray.length; i++) {
			...
			for (int j = 0; j < 3; j++) {
				Graphs.addEdge(currGraph, charArray[j], charArray[j + 1],
						weigthsArray[i][j]);
			}
		}

		// first assert all are isomorphic (if vertexChecker is not used)
		assertIsomorphic(new Graph[] { graphsArray[0], graphsArray[1] }, true);
		assertIsomorphic(new Graph[] { graphsArray[0], graphsArray[2] }, true);
		assertIsomorphic(new Graph[] { graphsArray[1], graphsArray[2] }, true);

		// create a functor according to odd even
		EquivalenceComparator edgeEqChecker = new DirectedEdgeWeightOddEvenComparator(
				graphsArray[0]);
		assertIsomorphic(new Graph[] { graphsArray[0], graphsArray[1] }, true,
				null, edgeEqChecker);
		assertIsomorphic(new Graph[] { graphsArray[0], graphsArray[2] }, false,
				null, edgeEqChecker);
		assertIsomorphic(new Graph[] { graphsArray[1], graphsArray[2] }, false,
				null, edgeEqChecker);
	}

	@SuppressWarnings("unchecked")
	private void assertIsomorphicStopAfterFirstMatch(Graph[] graphs,
			boolean assertActive, boolean shouldTheyBeIsomorphic,
			EquivalenceComparator vertexChecker,
			EquivalenceComparator edgeChecker) {
		if (assertActive) {
			System.out.println("\nassertIsomorphic:" + shouldTheyBeIsomorphic);
		...
		System.out.println("# Performence: Isomorphism check in MiliSeconds:"
				+ timeDesc);
		if (assertActive) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void performanceTestOnRandomGraphs() throws Exception {
		final int[] numOfVertexesArray = new int[] { 6, 6, 6, 8, 8, 8, 10, 10,
				10, 12, 14, 20, 30, 99 };
		final int[] numOfEdgesArray = new int[] { 0, 4, 12, 1, 15, 54, 0, 40,
				90, 130, 50, 79, 30, 234 };
		...
		RandomGraphGenerator[] genArray = new RandomGraphGenerator[NUM_OF_GENERATORS];

		String[] graphConctereClassesFullName = new String[] { // "org.jgrapht.graph.SimpleGraph"
																// ,
		...
		Graph[][] graphsArray = new Graph[graphConctereClassesFullName.length][SIZE_OF_GENERATED_GRAPH_ARRAY];
		...
		for (int testNum = 0; testNum < numOfVertexesArray.length; testNum++) {
			...
			try {
				for (int i = 0; i < graphConctereClassesFullName.length; i++) {
					for (int j = 0; j < SIZE_OF_GENERATED_GRAPH_ARRAY; j++) {
						graphsArray[i][j] = (Graph) Class.forName(
			...
			for (int i = 0; i < genArray.length; i++) {
				genArray[i] = new RandomGraphGenerator(
						numOfVertexesArray[testNum], numOfEdgesArray[testNum]);
			}

			for (int graphType = 0; graphType < graphConctereClassesFullName.length; graphType++) {
				System.out.println("### numOfVertexes= "
				...
				genArray[0].generateGraph(graphsArray[graphType][0],
						new IntegerVertexFactory(), null);
				genArray[0].generateGraph(graphsArray[graphType][1],
						new IntegerVertexFactory(), null);

				// 3rd from genArray[1]
				genArray[1].generateGraph(graphsArray[graphType][2],
						new IntegerVertexFactory(), null);
				...
				assertIsomorphicStopAfterFirstMatch(currIsoTestArray, true,
						true, null, null);

				// remember it is not a MUST - it can be true . DEGUG REASON
				// ONLY , and with care
				currIsoTestArray[1] = graphsArray[graphType][2];
				assertIsomorphicStopAfterFirstMatch(currIsoTestArray, false,
						false, null, null);
			}
		}
	}
