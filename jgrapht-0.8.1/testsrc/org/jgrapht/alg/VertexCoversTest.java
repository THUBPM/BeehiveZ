...
public class VertexCoversTest extends TestCase {
	...
	public void testFind2ApproximationCover() {
		for (int i = 0; i < TEST_REPEATS; i++) {
			...
			assertTrue(isCover(VertexCovers.find2ApproximationCover(g), g));
		}
	}
	...
	public void testFindGreedyCover() {
		for (int i = 0; i < TEST_REPEATS; i++) {
			Graph<Integer, DefaultEdge> g = createRandomGraph();
			Set<Integer> c = VertexCovers.findGreedyCover(Graphs
					.undirectedGraph(g));
	 ...
	 * 
	 * @param vertexSet
	 *            the vertices to be tested for covering the graph.
	 * @param g
	 *            the graph to be covered.
	 * 
	 * @return
	 */
	private boolean isCover(Set<Integer> vertexSet,
	...
	private Graph<Integer, DefaultEdge> createRandomGraph() {
		...
		// framework.
		Pseudograph<Integer, DefaultEdge> g = new Pseudograph<Integer, DefaultEdge>(
				DefaultEdge.class);
