...
public class DOTExporterTest extends TestCase {
	...
	private static final String UNDIRECTED = "graph G {" + NL + "  1;" + NL
			+ "  2;" + NL + "  3;" + NL + "  1 -- 2;" + NL + "  3 -- 1;" + NL
			+ "}" + NL;

	private static final DOTExporter<String, DefaultEdge> exporter = new DOTExporter<String, DefaultEdge>();
	...
	public void testValidNodeIDs() {
		...
		List<String> validVertices = Arrays.asList("-9.78", "-.5", "12", "a",
				"12", "abc_78", "\"--34asdf\"");
		for (String vertex : validVertices) {
			Graph<String, DefaultEdge> graph = new DefaultDirectedGraph<String, DefaultEdge>(
					DefaultEdge.class);
		...
		List<String> invalidVertices = Arrays.asList("2test", "--4", "foo-bar",
				"", "t:32");
